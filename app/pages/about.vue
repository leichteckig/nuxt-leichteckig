...
function getGithubProjects() {
  const ownRepoUrl = `${projectUrl}/repos?type=public`
  const ownStarsUrl = `${projectUrl}/starred`

  fetch(ownRepoUrl)
    .then(resp => resp.json())
    .then((data) => {
      projects.value = data.filter(project => !project.fork
    ...
    .then(() => {
      fetch(ownStarsUrl)
        .then(resp => resp.json())
        .then((data) => {
          const someStars = data.filter(project => project.full_name.includes('3stadt')
            || project.full_name.includes('platform')
            || project.full_name.includes('FriendsOfShopware')
          )
          projects.value = [...projects.value, ...someStars]
        })
    })
}

onMounted(getGithubProjects)
</script>
